...
Remora has received a Windows two-alias pilot under normal provider approvals, but broader authenticated validation remains incomplete. Treat the local dashboard token, SQLite home, task snapshots, logs, and provider profiles as sensitive. Keep the service bound to localhost, use dedicated provider profiles, review staged results before acceptance, and remove test snapshots when finished.
